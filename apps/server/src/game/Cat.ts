...
const CAT_SPEED = 38; // px/s — cats mosey
const CAT_HITBOX = { x: 3, y: 6, width: 10, height: 8 };
...
/** A server-authoritative NPC cat that wanders the map on its own. */
export class Cat {
  x: number;
  y: number;
  dir: Dir = "down";
  ...
  private tryMove(dt: number, isSolid: SolidTest): boolean {
    const { width, height, x: hx, y: hy } = CAT_HITBOX;
    let moved = false;
    const mx = this.dx * CAT_SPEED * dt;
    const my = this.dy * CAT_SPEED * dt;
    if (mx !== 0) {
      const nx = this.x + mx;
